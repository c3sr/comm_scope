...
auto Comm_ZeroCopy_GPUGPU = [](benchmark::State &state, const int gpu0, const int gpu1, const AccessType access_type) {
  if (!has_cuda) {
...
#define WR_INIT(src, dst, op_idx)                                                                                      \
  ...
  // time is computed from stream 0, which does not complete until stream 1 is done
  OR_SKIP(cudaSetDevice(gpu0));
...
#define READ_ITER(dst, op_idx)                                                                                         \
  {                                                                                                                    \
    OR_SKIP(cudaSetDevice(gpu##dst));                                                                                  \
    gpu_read<int32_t><<<256, 256, 0, streams[dst]>>>(static_cast<int32_t *>(ptrs[op_idx]), bytes);                     \
  }
#define WRITE_ITER(src, op_idx)                                                                                        \
  {                                                                                                                    \
    OR_SKIP(cudaSetDevice(gpu##src));                                                                                  \
    gpu_write<int32_t><<<256, 256, 0, streams[src]>>>(static_cast<int32_t *>(ptrs[op_idx]), bytes);                    \
  }
        ...
        if (src_gpu < dst_gpu) {
          ...
          if (!PRINT_IF_ERROR(cudaDeviceCanAccessPeer(&s2d, src_gpu, dst_gpu)) &&
            ...
            if (s2d && d2s) {
              ...
              name += to_string(workload) + "/" + std::to_string(src_gpu) + "/" + std::to_string(dst_gpu);
              benchmark::RegisterBenchmark(name.c_str(), Comm_ZeroCopy_GPUGPU, src_gpu, dst_gpu, workload)
                  ->ARGS()
                  ->UseManualTime();
            } else {
