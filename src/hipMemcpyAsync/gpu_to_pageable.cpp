...
auto Comm_hipMemcpyAsync_GPUToPageable = [](benchmark::State &state, const int numaId, const int hipId, const bool flush) {
    ...
    for (const auto &numaSpace : numaSpaces) {
      ...
      if (numa::can_execute_in_node(numaId)) {
        name = std::string(NAME) + "/" + std::to_string(numaId) + "/" +
              std::to_string(hipId);
        benchmark::RegisterBenchmark(name.c_str(), Comm_hipMemcpyAsync_GPUToPageable,
                                    numaId, hipId, false)
            ->SMALL_ARGS()
            ->UseManualTime();
