...
auto Comm_cudaMemcpyAsync_GPUToPinned = [](benchmark::State &state,
  ...
  for (const auto &cudaSpace : cudaSpaces) {
    for (const auto &numaSpace : numaSpaces) {
      ...
      benchmark::RegisterBenchmark(
          name.c_str(), Comm_cudaMemcpyAsync_GPUToPinned, numaId, cudaId, false)
          ->SMALL_ARGS()
          ->UseManualTime();
      name = std::string(NAME) + "_flush/" + std::to_string(numaId) + "/" +
             std::to_string(cudaId);
      benchmark::RegisterBenchmark(
          name.c_str(), Comm_cudaMemcpyAsync_GPUToPinned, numaId, cudaId, true)
          ->SMALL_ARGS()
          ->UseManualTime();
    }
  }
