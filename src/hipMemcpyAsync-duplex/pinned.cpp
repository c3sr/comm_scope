...
auto Comm_hipMemcpyAsync_Duplex_Pinned =
...
static void registerer() {
  ...
  for (auto hipSpace : hipSpaces) {
    for (auto numaSpace : numaSpaces) {
      ...
      transfers.push_back(
          new PinnedCopyConfig(hipMemcpyHostToDevice, numa_id, hip_id));
      transfers.push_back(
          new PinnedCopyConfig(hipMemcpyDeviceToHost, hip_id, numa_id));
      name = std::string(NAME) + "/" + std::to_string(numa_id) + "/" +
             std::to_string(hip_id);
      benchmark::RegisterBenchmark(
          name.c_str(), Comm_hipMemcpyAsync_Duplex_Pinned, transfers, false)
          ->SMALL_ARGS()
          ->UseManualTime();
      name = std::string(NAME) + "_flush" + "/" + std::to_string(numa_id) +
             "/" + std::to_string(hip_id);
      benchmark::RegisterBenchmark(
          name.c_str(), Comm_hipMemcpyAsync_Duplex_Pinned, transfers, true)
          ->SMALL_ARGS()
          ->UseManualTime();
    }
  }
