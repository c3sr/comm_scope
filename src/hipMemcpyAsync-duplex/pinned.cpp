...
#define NAME "Comm_hipMemcpyAsync_Duplex_Pinned" 

auto Comm_hipMemcpyAsync_Duplex_Pinned = [](benchmark::State &state, std::vector<HipMemcpyConfig*> transfers, const bool flush) {
  ...
  for (auto _ : state) {
    ...
    for (const auto config : transfers) {
      ...
      OR_SKIP_AND_BREAK(hipEventRecord(start, stream), NAME " failed to record start event");
      OR_SKIP_AND_BREAK(hipMemcpyAsync(dst, src, bytes, hipMemcpyDefault, stream), NAME " failed to start hipMemcpyAsync");
      OR_SKIP_AND_BREAK(hipEventRecord(stop, stream), NAME " failed to record stop event");
    }

    // Wait for all copies to finish
    for (const auto config : transfers) {
      auto stop = config->stop_;
      OR_SKIP_AND_BREAK(hipEventSynchronize(stop), NAME " failed to synchronize");
    }
    ...
    for (const auto config_start : transfers) {
      for (const auto config_stop : transfers) {
        ...
        float millis;
        OR_SKIP_AND_BREAK(hipEventElapsedTime(&millis, start, stop), NAME " failed to compute elapsed tiume");
...
static void registerer() {
  ...
  for (auto hipSpace : hipSpaces) {
    for (auto numaSpace : numaSpaces) {
      ...
      transfers.push_back(new PinnedCopyConfig(hipMemcpyHostToDevice, numa_id, hip_id));
      transfers.push_back(new PinnedCopyConfig(hipMemcpyDeviceToHost, hip_id, numa_id));
      name = std::string(NAME) 
           + "/" + std::to_string(numa_id) 
           + "/" + std::to_string(hip_id);
      benchmark::RegisterBenchmark(name.c_str(), Comm_hipMemcpyAsync_Duplex_Pinned, transfers, false)->SMALL_ARGS()->UseManualTime();
      name = std::string(NAME) + "_flush"
           + "/" + std::to_string(numa_id) 
           + "/" + std::to_string(hip_id);
      benchmark::RegisterBenchmark(name.c_str(), Comm_hipMemcpyAsync_Duplex_Pinned, transfers, true)->SMALL_ARGS()->UseManualTime();
    }
  }
