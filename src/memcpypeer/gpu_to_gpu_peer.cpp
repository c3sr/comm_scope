...
#define NAME "Comm_MemcpyPeer_Peer"
...
auto Comm_MemcpyPeer_Peer = [](benchmark::State &state, const int srcGpu, const int dstGpu) {
  if (!has_cuda) {
    ...
    for (size_t j = i; j < unique_cuda_device_ids().size(); ++j) {
      ...
      if (!PRINT_IF_ERROR(cudaDeviceCanAccessPeer(&s2d, srcGpu, dstGpu)) &&
        ...
        if (s2d && d2s) {
          ...
          benchmark::RegisterBenchmark(name.c_str(), Comm_MemcpyPeer_Peer, srcGpu, dstGpu)
              ->SMALL_ARGS()
              ->UseManualTime();
        }
      }
