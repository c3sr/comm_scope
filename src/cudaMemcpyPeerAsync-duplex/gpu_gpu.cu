...
auto Comm_cudaMemcpyPeerAsync_Duplex_GPUGPU = [](benchmark::State &state,
  ...
  OR_SKIP_AND_RETURN(cudaMalloc(&src0, bytes),
                     NAME " failed to perform src0 cudaMalloc");
  OR_SKIP_AND_RETURN(cudaMalloc(&dst0, bytes),
                     NAME " failed to perform src1 cudaMalloc");
  defer(cudaFree(src0));
  defer(cudaFree(dst0));
  ...
  OR_SKIP_AND_RETURN(cudaMalloc(&dst1, bytes),
                     NAME " failed to perform dst1 cudaMalloc");
  defer(cudaFree(src1));
  defer(cudaFree(dst1));
  ...
  for (auto _ : state) {
    ...
    restart_iteration: // use a label here instead of a nested loop so OR_SKIP_AND_BREAK breaks out of benchmark loop
      ...
      OR_SKIP_AND_BREAK(
          ...
          NAME " failed to memcpy");
      OR_SKIP_AND_BREAK(cudaEventRecord(stop1, stream1),
                        NAME " failed to stop");
      OR_SKIP_AND_BREAK(cudaSetDevice(gpu0), NAME " failed to set src device");
