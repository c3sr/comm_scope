...
#define NAME "Comm_3d_cudaMemcpy2DAsync"
...
auto Comm_3d_cudaMemcpy2DAsync = [](benchmark::State &state, const int gpu0, const int gpu1) {
  if (!has_cuda) {
  ...
  OR_SKIP(cudaMemset3D(dst, 0, allocExt), NAME " failed to perform dst cudaMemset");
  if (gpu0 != gpu1) {
    ...
    for (size_t j = i; j < unique_cuda_device_ids().size(); ++j) {
      ...
      if (!PRINT_IF_ERROR(cudaDeviceCanAccessPeer(&ok1, gpu0, gpu1)) &&
        ...
        if ((ok1 && ok2) || i == j) {
          ...
          benchmark::RegisterBenchmark(name.c_str(), Comm_3d_cudaMemcpy2DAsync, gpu0, gpu1)
              ->TINY_ARGS()
              ->UseManualTime();
        }
      }
