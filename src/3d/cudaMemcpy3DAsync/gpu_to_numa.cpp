...
#define NAME "Comm_3d_cudaMemcpy3DAsync_GPUToNUMA"
  ...
  #if SCOPE_USE_NVTX == 1
{
  ...
  for (auto _ : state) {
    ...
    OR_SKIP_LOOP(cudaMemcpy3DAsync(&params, stream), " failed to start cudaMemcpy3DAsync");
...
for (auto cudaId : unique_cuda_device_ids()) {
    for (auto numaId : unique_numa_ids()) {
          ...
          benchmark::RegisterBenchmark(name.c_str(), Comm_3d_cudaMemcpy3DAsync_GPUToNUMA, numaId, cudaId)
              ->TINY_ARGS()
              ->UseManualTime();
