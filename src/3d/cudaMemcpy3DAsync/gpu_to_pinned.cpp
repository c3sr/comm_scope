...
auto Comm_3d_cudaMemcpy3DAsync_GPUToPinned = [](benchmark::State &state,
                                                const int numaId,
                                                const int cudaId) {
  ...
  for (auto _ : state) {
    ...
    OR_SKIP_AND_BREAK(cudaMemcpy3DAsync(&params, stream),
                      " failed to start cudaMemcpy3DAsync");
  ...
  for (const auto &cudaSpace : cudaSpaces) {
    for (const auto &numaSpace : numaSpaces) {
      ...
      benchmark::RegisterBenchmark(
          name.c_str(), Comm_3d_cudaMemcpy3DAsync_GPUToPinned, numaId, cudaId)
          ->ASTAROTH_ARGS()
          ->UseManualTime();
    }
  }
