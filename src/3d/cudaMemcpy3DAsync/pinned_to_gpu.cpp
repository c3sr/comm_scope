...
#define NAME "Comm_3d_cudaMemcpy3DAsync_PinnedToGPU"

auto Comm_3d_cudaMemcpy3DAsync_PinnedToGPU = [](benchmark::State &state,
                                                const int numaId,
                                                const int cudaId) {
  ...
  src.ysize = dst.ysize;
  OR_SKIP_AND_RETURN(
      cudaHostRegister(src.ptr, allocBytes, cudaHostRegisterPortable),
      "cudaHostRegister()");
  ...
  for (auto _ : state) {
    ...
    OR_SKIP_AND_BREAK(cudaMemcpy3DAsync(&params, stream),
                      NAME " failed to start cudaMemcpy3DAsync");
  ...
  for (const auto &cudaSpace : cudaSpaces) {
    for (const auto &numaSpace : numaSpaces) {
      ...
      benchmark::RegisterBenchmark(
          name.c_str(), Comm_3d_cudaMemcpy3DAsync_PinnedToGPU, numaId, cudaId)
          ->ASTAROTH_ARGS()
          ->UseManualTime();
    }
  }
