...
#define NAME "Comm_3d_cudaMemcpy3DPeerAsync"
...
auto Comm_3d_cudaMemcpy3DPeerAsync = [](benchmark::State &state, const int gpu0, const int gpu1) {
  if (!has_cuda) {
  ...
  #if SCOPE_USE_NVTX == 1
{
  ...
  name << NAME << "/" << gpu0 << "/" << gpu1  << "/" << state.range(0) << "/" << state.range(1) << "/" << state.range(2);
  ...
  OR_SKIP(cudaMemset3D(src, 0, allocExt), NAME " failed to perform src cudaMemset");
  if (gpu0 != gpu1) {
  ...
  OR_SKIP(cudaMemset3D(dst, 0, allocExt), NAME " failed to perform src cudaMemset");
  if (gpu0 != gpu1) {
  ...
  for (auto _ : state) {
    ...
    OR_SKIP_LOOP(cudaMemcpy3DPeerAsync(&params, stream), NAME " failed to start cudaMemcpy3DPeerAsync");
    ...
    for (size_t j = i; j < unique_cuda_device_ids().size(); ++j) {
      ...
      if (!PRINT_IF_ERROR(cudaDeviceCanAccessPeer(&ok1, gpu0, gpu1)) &&
        ...
        if ((ok1 && ok2) || i == j) {
          ...
          benchmark::RegisterBenchmark(name.c_str(), Comm_3d_cudaMemcpy3DPeerAsync, gpu0, gpu1)
              ->TINY_ARGS()
              ->UseManualTime();
        }
      }
