...
#define NAME "Comm_3d_cudaMemcpy2DAsync_GPUToNUMA"
...
auto Comm_3d_cudaMemcpy2DAsync_GPUToNUMA = [](benchmark::State &state, const int numaId, const int cudaId) {
  ...
  #if SCOPE_USE_NVTX == 1
{
  ...
  dst.xsize = src.xsize;
  dst.ysize = src.ysize;
  OR_SKIP(cudaHostRegister(dst.ptr, allocBytes, cudaHostRegisterPortable), "cudaHostRegister()");
  std::memset(dst.ptr, 0, allocBytes);

  for (auto _ : state) {
    // Start copy
    OR_SKIP_LOOP(cudaEventRecord(start, stream), " failed to record start event");
    ...
    for (size_t z = 0; z < copyExt.depth; ++z) {
      ...
      OR_SKIP_LOOP(cudaMemcpy2DAsync(
          dstP, allocExt.width, srcP, allocExt.width, copyExt.width, copyExt.height, cudaMemcpyDeviceToHost, stream), NAME " failed to start 2D copy");
    }
    OR_SKIP_LOOP(cudaEventRecord(stop, stream), " failed to record stop event");

    // Wait for all copies to finish
    OR_SKIP_LOOP(cudaEventSynchronize(stop), "failed to synchronize");

    // Get the transfer time
    float millis;
    OR_SKIP_LOOP(cudaEventElapsedTime(&millis, start, stop), "failed to compute elapsed time");
...
for (auto cudaId : unique_cuda_device_ids()) {
    for (auto numaId : unique_numa_ids()) {
          ...
          benchmark::RegisterBenchmark(name.c_str(), Comm_3d_cudaMemcpy2DAsync_GPUToNUMA, numaId, cudaId)
              ->TINY_ARGS()
              ->UseManualTime();
