...
auto Comm_3d_cudaMemcpy2DAsync_GPUToNUMA = [](benchmark::State &state,
                                              const int numaId,
                                              const int cudaId) {
  ...
  dst.xsize = src.xsize;
  dst.ysize = src.ysize;
  OR_SKIP(cudaHostRegister(dst.ptr, allocBytes, cudaHostRegisterPortable),
          "cudaHostRegister()");
  ...
  for (auto cudaId : unique_cuda_device_ids()) {
    for (auto numaId : numa::ids()) {
      ...
      benchmark::RegisterBenchmark(
          name.c_str(), Comm_3d_cudaMemcpy2DAsync_GPUToNUMA, numaId, cudaId)
          ->TINY_ARGS()
          ->UseManualTime();
    }
  }
