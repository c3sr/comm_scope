...
#define NAME "Comm_3d_kernel3D_NUMAToGPU"

__global__ void Comm_3d_kernel3D_NUMAToGPU_kernel(void *__restrict__ dst, const void *__restrict__ src,
...
auto Comm_3d_kernel3D_NUMAToGPU = [](benchmark::State &state, const int numaId, const int cudaId) {
    ...
    #if SCOPE_USE_NVTX == 1
  {
    ...
    src.xsize = dst.xsize;
    src.ysize = dst.ysize;
    OR_SKIP(cudaHostRegister(src.ptr, allocBytes, cudaHostRegisterPortable | cudaHostRegisterMapped), "cudaHostRegister()");
  ...
  gridDim.z = (copyExt.depth + blockDim.z - 1) / blockDim.z;
    ...
    for (auto _ : state) {
      ...
      Comm_3d_kernel3D_NUMAToGPU_kernel<<<gridDim, blockDim, 0, stream>>>(dst.ptr, src.ptr, allocExt, copyExt, elemSize);
...
static void registerer() {
  ...
  for (auto cudaId : unique_cuda_device_ids()) {
      for (auto numaId : unique_numa_ids()) {
            ...
            benchmark::RegisterBenchmark(name.c_str(), Comm_3d_kernel3D_NUMAToGPU, numaId, cudaId)
                ->TINY_ARGS()
                ->UseManualTime();
