...
#define NAME "Comm_3d_kernel3D_pull"

__global__ void Comm_3d_kernel3D_pull_kernel(void *__restrict__ dst, const void *__restrict__ src,
...
auto Comm_3d_kernel3D_pull = [](benchmark::State &state, const int gpu0, const int gpu1) {
  ...
  for (auto _ : state) {
    ...
    Comm_3d_kernel3D_pull_kernel<<<gridDim, blockDim, 0, stream>>>(dst.ptr, src.ptr, allocExt, copyExt, elemSize);
