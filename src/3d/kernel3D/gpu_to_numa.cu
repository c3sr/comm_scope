...
#define NAME "Comm_3d_kernel3D_GPUToNUMA"

__global__ void Comm_3d_kernel3D_GPUToNUMA_kernel(void *__restrict__ dst, const void *__restrict__ src,
...
auto Comm_3d_kernel3D_GPUToNUMA = [](benchmark::State &state, const int numaId, const int cudaId) {
    ...
    #if SCOPE_USE_NVTX == 1
  {
    ...
    dst.xsize = src.xsize;
    dst.ysize = src.ysize;
    OR_SKIP(cudaHostRegister(dst.ptr, allocBytes, cudaHostRegisterPortable | cudaHostRegisterMapped), "cudaHostRegister()");
  ...
  gridDim.z = (copyExt.depth + blockDim.z - 1) / blockDim.z;
    ...
    for (auto _ : state) {
      ...
      Comm_3d_kernel3D_GPUToNUMA_kernel<<<gridDim, blockDim, 0, stream>>>(dst.ptr, src.ptr, allocExt, copyExt, elemSize);
...
static void registerer() {
  ...
  for (auto cudaId : unique_cuda_device_ids()) {
      for (auto numaId : unique_numa_ids()) {
            ...
            benchmark::RegisterBenchmark(name.c_str(), Comm_3d_kernel3D_GPUToNUMA, numaId, cudaId)
                ->TINY_ARGS()
                ->UseManualTime();
