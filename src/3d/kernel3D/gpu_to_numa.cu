...
__global__ void Comm_3d_kernel3D_GPUToNUMA_kernel(void *__restrict__ dst,
...
auto Comm_3d_kernel3D_GPUToNUMA = [](benchmark::State &state, const int numaId,
  ...
  for (auto _ : state) {
    ...
    Comm_3d_kernel3D_GPUToNUMA_kernel<<<gridDim, blockDim, 0, stream>>>(
...
static void registerer() {
  ...
  for (auto cudaId : unique_cuda_device_ids()) {
    for (auto numaId : numa::ids()) {
      ...
      benchmark::RegisterBenchmark(name.c_str(), Comm_3d_kernel3D_GPUToNUMA,
                                   numaId, cudaId)
          ->TINY_ARGS()
          ->UseManualTime();
    }
