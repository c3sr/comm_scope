...
auto Comm_implicit_mapped_GPUWrHost = [](benchmark::State &state, 
                                        const Device &wr_gpu,
                                        const MemorySpace &own_numa,
                                        bool flush) {
    ...
    if (hip.can_map_host_memory()) {
      for (const MemorySpace &numa : scope::system::numa_memory_spaces()) {
        for (const bool flush : {false, true}) {
          std::string name = std::string(NAME) + "/" + std::to_string(hip.device_id()) +
                            "/" + std::to_string(numa.numa_id());
          if (flush) {
            name += "/flush";
          }
          benchmark::RegisterBenchmark(name.c_str(), Comm_implicit_mapped_GPUWrHost,
                                      hip, numa, flush)
              ->SMALL_ARGS()
              ->UseManualTime();
        }
