...
class CudaMemcpyConfig {
  ...
  cudaError_t reset_gpu() {
      if (cudaMemcpyHostToDevice == kind_) {
          return utils::cuda_reset_device(dst_id_);
      } else if (cudaMemcpyDeviceToHost == kind_) {
          return utils::cuda_reset_device(src_id_);
      } else {
          assert(0 && "Unexpected kind_");
      }
      return cudaSuccess;  
  ...
  public:
    ...
    cudaError_t  cpu_allocate(const size_t bytes) {
        if (cudaMemcpyHostToDevice == kind_) {
            numa_bind_node(src_id_);
            src_ = aligned_alloc(page_size(), bytes);
            OR_RETURN(cudaHostRegister(src_, bytes, cudaHostRegisterPortable | cudaHostRegisterMapped));
            std::memset(src_, 0, bytes);
        } else if (cudaMemcpyDeviceToHost == kind_) {
            numa_bind_node(dst_id_);
            dst_ = aligned_alloc(page_size(), bytes);
            OR_RETURN(cudaHostRegister(dst_, bytes, cudaHostRegisterPortable | cudaHostRegisterMapped));
            std::memset(dst_, 0, bytes);
        } else {
            assert(0 && "Unexpected kind_");
        }
