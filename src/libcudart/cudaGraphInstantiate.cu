...
#define NAME "Comm_cudart_cudaGraphInstantiate"

__global__ void Comm_cudart_cudaGraphInstantiate_kernel() {}
...
static void kernel_launcher(cudaStream_t stream, const int iters) {
  for (int i = 0; i < iters; ++i) {
    Comm_cudart_cudaGraphInstantiate_kernel<<<1, 1, 0, stream>>>();
  }
}

auto Comm_cudart_cudaGraphInstantiate = [](benchmark::State &state,
                                           const int numa_id,
                                           const int cuda_id) {
  ...
  for (auto cuda_id : unique_cuda_device_ids()) {
    for (auto numa_id : numa::ids()) {
      ...
      benchmark::RegisterBenchmark(
          name.c_str(), Comm_cudart_cudaGraphInstantiate, numa_id, cuda_id)
          ->GRAPH_ARGS();
    }
  }
}
