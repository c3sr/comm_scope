/* Measure the runtime cost of cudaMemcpy3DAsync
 */

#include "scope/scope.hpp"

#define NAME "Comm_cudart_cudaMemcpy3DAsync_GPUToGPU"

// all threads use the same memcpy params and the same stream
static cudaStream_t gStream;
static cudaMemcpy3DParms gParams;

auto Comm_cudart_cudaMemcpy3DAsync = [](benchmark::State &state, const int gpu0,
  ...
  OR_SKIP_AND_RETURN(err, "failed to cudaMemcpy3DAsync");
        ...
        if (!PRINT_IF_ERROR(cudaDeviceCanAccessPeer(&ok1, gpu0, gpu1)) &&
          ...
          if ((ok1 && ok2) || i == j) {
            ...
            benchmark::RegisterBenchmark(
                name.c_str(), Comm_cudart_cudaMemcpy3DAsync, gpu0, gpu1,
