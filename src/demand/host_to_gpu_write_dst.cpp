...
auto Comm_hipManaged_HostToGPUWriteDst = [](benchmark::State &state,
                                            const int src_numa,
                                            const int dst_gpu) {
  const auto bytes = 1ULL << static_cast<size_t>(state.range(0));
  UnaryData data =
      setup<Kind::HostToGPU>(state, NAME, bytes, src_numa, dst_gpu);
    ...
    for (const MemorySpace &hs : hipSpaces) {
      ...
      if (numa::can_execute_in_node(src_numa)) {
        ...
        benchmark::RegisterBenchmark(
            name.c_str(), Comm_hipManaged_HostToGPUWriteDst, src_numa, dst_gpu)
            ->SMALL_ARGS()
            ->UseManualTime();
      }
    }
