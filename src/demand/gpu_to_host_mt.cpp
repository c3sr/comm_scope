...
auto Comm_hipManaged_GPUToHostMt = [](benchmark::State &state,
                                      const int src_gpu, const int dst_numa,
                                      const int num_threads) {
  const auto bytes = 1ULL << static_cast<size_t>(state.range(0));
  UnaryData data =
      setup<Kind::GPUToHost>(state, NAME, bytes, src_gpu, dst_numa);
  ...
  for (auto _ : state) {
    ...
    for (int i = 0; i < num_threads; ++i) {
      workers[i] = std::thread(cpu_write, &data.ptr[i * bytes / num_threads],
                               bytes / num_threads, CACHE_LINE_SIZE, &starts[i],
                               &stops[i]);
    }
    ...
    for (const auto threadStart : starts) {
      for (const auto threadStop : stops) {
        auto threadElapsed = scope::duration(threadStop - threadStart).count();
      ...
      for (const MemorySpace &hs : hipSpaces) {
        ...
        if (numa::can_execute_in_node(dst_numa)) {
          ...
          benchmark::RegisterBenchmark(name.c_str(),
                                       Comm_hipManaged_GPUToHostMt, src_gpu,
                                       dst_numa, num_threads)
              ->MT_ARGS()
              ->UseManualTime()
              ->MinTime(0.005);
        }
