...
auto Comm_hipManaged_Prefetch_GPUToGPU =
    [](benchmark::State &state, const int src_gpu, const int dst_gpu) {
  ...
  for (size_t i = 0; i < hips.size(); ++i) {
    for (size_t j = i + 1; j < hips.size(); ++j) {
      ...
      benchmark::RegisterBenchmark(
          name.c_str(), Comm_hipManaged_Prefetch_GPUToGPU, src_gpu, dst_gpu)
          ->SMALL_ARGS()
          ->UseManualTime();
    }
  }
